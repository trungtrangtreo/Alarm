package search.test.androidhive.alarmcustom.ui;
...
import search.test.androidhive.alarmcustom.R;
import search.test.androidhive.alarmcustom.activity.MainActivity;
                    ...
                    onSeekBarChange((v, progress, fromUser) -> {
                        if (fromUser) {
                            if (App.getState().settings().snap()) {
                                progress = (int) (Math.round(progress / 1.0) * 1) % 60;
                            }
